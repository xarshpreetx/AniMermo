...
object RepositoryManager {
    ...
    val PREBUILT_REPOSITORIES: Array<RepositoryData> by lazy {
        // Sempre retorna os repositórios fixos + quaisquer adicionais do usuário
        FixedRepositories.FIXED_REPOS + (getKey<Array<RepositoryData>>("PREBUILT_REPOSITORIES") ?: emptyArray())
    }
    ...
    fun getRepositories(): Array<RepositoryData> {
        // Sempre inclui os repositórios fixos
        val userRepos = getKey<Array<RepositoryData>>(REPOSITORIES_KEY) ?: emptyArray()
        return (FixedRepositories.FIXED_REPOS + userRepos).distinctBy { it.url }.toTypedArray()
    }
    ...
    suspend fun removeRepository(context: Context, repository: RepositoryData) {
        // Impede remoção de repositórios fixos
        if (FixedRepositories.FIXED_REPOS.any { it.url == repository.url }) {
            return // Não permite remover repositórios fixos
        }
