...
object PluginManager {
    ...
    suspend fun ___DO_NOT_CALL_FROM_A_PLUGIN_loadAllOnlinePlugins(context: Context) {
        ...
        // Auto-download providers fixos caso não estejam instalados
        ___DO_NOT_CALL_FROM_A_PLUGIN_ensureFixedProvidersInstalled(context)
    }
    
    /**
     * Garante que os providers fixos (AnimeFire e outros) estejam instalados
     * Esta função é chamada automaticamente ao carregar os plugins online
     */
    @Suppress("FunctionName")
    private suspend fun ___DO_NOT_CALL_FROM_A_PLUGIN_ensureFixedProvidersInstalled(context: Context) {
        try {
            // Busca todos os plugins disponíveis nos repositórios fixos
            val onlinePlugins = FixedRepositories.FIXED_REPOS.toList().amap {
                getRepoPlugins(it.url)?.toList() ?: emptyList()
            }.flatten()
            
            // Filtra apenas os providers que estão na lista de auto-download
            val fixedProviders = onlinePlugins.filter { (_, plugin) ->
                FixedRepositories.AUTO_DOWNLOAD_PROVIDERS.contains(plugin.internalName)
            }
            
            // Baixa e instala os providers que ainda não estão instalados
            fixedProviders.forEach { (repoUrl, plugin) ->
                val pluginPath = getPluginPath(context, plugin.internalName, repoUrl)
                
                if (!pluginPath.exists()) {
                    Log.i(TAG, "Auto-installing fixed provider: ${plugin.name}")
                    downloadPlugin(
                        context as? Activity ?: return@forEach,
                        plugin.url,
                        plugin.internalName,
                        repoUrl,
                        true // Habilitar automaticamente
                    )
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error ensuring fixed providers: ${e.message}", e)
        }
    }
